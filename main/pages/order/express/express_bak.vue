	...
	<view class="express-info" v-if="!loadding">
		<view class="base-info p30">
			<view class="name">
				...
				<text class="fb">{{express.list.expName}}</text>
			</view>
		...
		<view class="list">
			<view :class="index==0?'active item':'item'" v-for="(item, index) in express.list.list" :key="index">
...
<script>
	export default {
		...
		methods: {
			/*获取数据*/
			getData() {
				// this._post('plus.coc.Coc/getExpressByCode', {
				// 	logistic_code: 'JT5273312904239'
				// }, function(res) {
