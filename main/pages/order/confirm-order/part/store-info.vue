<template>
	<div>
		...
		<template v-else>
			<view class="address-defalut-wrap">
				<view class="info d-s-s">
					<text class="state">{{$nw("当前自提点")}}</text>
		...
		<view class="buy-checkout vender">
			<view class="group-hd">
				<view class="left"><text class="min-name">{{$nw("提货人信息")}}</text></view>
			</view>
			<view class="item">
				<text class="key">{{$nw("联系人")}}：</text>
				<input type="text" v-model="linkman" class="flex-1 f30" placeholder-class="gray"
				:placeholder="$nw('请输入联系人')" value="" />
			</view>
			<view class="item">
				<text class="key">{{$nw("联系电话")}}：</text>
				<input type="text" v-model="phone" class="flex-1 f30" placeholder-class="gray" 
				:placeholder="$nw('请输入联系电话')" value="" />
			</view>
		</view>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		addAddress() {
			...
			uni.navigateTo({
				url: '/pages/store/address/address?store_id=' + store_id + '&is_select=1'
			});
		}
	}
};
