		...
		<view class="uni-product-list">
			<view class="uni-product" v-for="(product,index) in productList" :key="index" @click="goDetailPage(product.product_id)">
				<view class="image-view">
					...
					<view class="number" v-if="product.sku[0].stock_num < 100000">{{$nw("库存")}}：{{product.sku[0].stock_num}}</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			goDetailPage(product_id) {
				uni.navigateTo({
					url: '/main/pages/order/order-detail/order-detail?product_id='+product_id
				});
			}
		}
	}
