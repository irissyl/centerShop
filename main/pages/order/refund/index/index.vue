<template>
	<view class="refund-list">
		<view class="top-tabbar">
			...
			:style="{'color':state_active==-1?getTabbarSelectColor():getTabbarColor()}"
			
			@click="stateFunc(-1)">
				{{$nw("全部")}}
			</view>
			<view :class="state_active==0?'tab-item active':'tab-item'" 
			:style="{'color':state_active==0?getTabbarSelectColor():getTabbarColor()}"
			 @click="stateFunc(0)">
				{{$nw("待处理")}}
			</view>
		</view>
			...
			<view class="list">
				<view class="item bg-white p30 mb20" v-for="(item,index) in tableData" :key="index">
					...
					<view class="d-e-c pt20">
						<view>
							{{$nw("商品金额")}}：
							<text class="red">{{$nw("¥")}}{{item.order_master.total_price}}</text>
						</view>
					</view>
					<view class="d-e-c pt10">
						<view>
							{{$nw("订单实付金额")}}：
							<text class="red">{{$nw("¥")}}{{item.order_master.pay_price}}</text>
						</view>
					</view>
					<view class="d-e-c mt20 pt20 border-t">
						<button type="default" class="btn-gray-border" @click="gotoRefundDetail(item.order_refund_id)">{{$nw("查看详情")}}</button>
					</view>
				</view>
		...
		methods: {
			...
			getData() {
				...
				self._get('user.refund/lists', {
					...
					self.tableData = self.getUnique(self.tableData,"order_refund_id");
			...
			gotoRefundDetail(e) {
				uni.navigateTo({
					url: '/main/pages/order/refund/detail/detail?order_refund_id=' + e,
				});
			},
			/*可滚动视图区域到顶触发*/
			scrolltoupperFunc() {
