	...
	<view class="look-evaluate">
		<view class="top-tabbar">
			<view :class="state_active == -1 ? 'tab-item active' : 'tab-item'" :style="[active(state_active, -1)]"
				@click="stateFunc(-1)">{{$nw("全部")}} <text v-if="Total.all.length > 0"> ({{ Total.all }})</text></view>
			<view :class="state_active == 10 ? 'tab-item active' : 'tab-item'" 
			:style="[active(state_active, 10)]"
				@click="stateFunc(10)">
				<view class="d-c-c mr20">
					<text class="icon iconfont icon-pingjiahaoping"></text>
					<text class="ml10 gray9">{{$nw("好评")}}<text v-if="Total.praise.length > 0">({{ Total.praise }})</text></text>
				</view>
			</view>
			<view :class="state_active == 20 ? 'tab-item active' : 'tab-item'" :style="[active(state_active, 20)]"
				@click="stateFunc(20)">
				<view class="d-c-c mr20">
					<text class="icon iconfont icon-pingjiazhongping"></text>
					<text class="ml10 gray9">{{$nw("中评")}}<text v-if="Total.review.length > 0">({{ Total.review }})</text></text>
				</view>
			</view>
			<view :class="state_active == 30 ? 'tab-item active' : 'tab-item'" :style="[active(state_active, 30)]"
				@click="stateFunc(30)">
				<view class="d-c-c mr20">
					<text class="icon iconfont icon-pingjiachaping"></text>
					<text class="ml10 gray9">{{$nw("差评")}}<text v-if="Total.negative.length > 0">({{ Total.negative }})</text></text>
				</view>
			</view>
		</view>
					...
					<view class="d-b-c p20">
						<view class="d-s-c">
							<view v-if="item.score == 10" class="d-c-c mr20">
								<text class="icon iconfont icon-pingjiahaoping"></text>
								<text class="ml10 gray9">{{$nw("好评")}}</text>
							</view>
							<view v-if="item.score == 20" class="d-c-c mr20">
								<text class="icon iconfont icon-pingjiazhongping"></text>
								<text class="ml10 gray9">{{$nw("中评")}}</text>
							</view>
							<view v-if="item.score == 30" class="d-c-c mr20">
								<text class="icon iconfont icon-pingjiachaping"></text>
								<text class="ml10 gray9">{{$nw("差评")}}</text>
							</view>
						</view>
				...
				return (state_active, value) => {
					if (state_active === value) {
						return Object.assign({
							...
							'border-bottom': '2px solid ' + this.getMainTextColor()
						})
					}
				}
			},
			...
			getData() {
				...
				self._get(
					...
					function(res) {
						...
						self.tableData = self.getUnique(self.tableData, "comment_id");
