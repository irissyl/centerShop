<template>

	<view class="bottom-panel" :class="Visible?'bottom-panel open':'bottom-panel close'" @click="closePopup">
		<view class="popup-bg">
		</view>

		<view class="content" v-on:click.stop>

			<view class="module-box module-share">
				<view class="hd d-c-c">
					{{$nw("产品参数")}}
				</view>
				<view class="product-wraper" v-for="(item, index) in list" :key="index">
					<view class="product-item">
						<view>{{item.attr_value_name}}</view>
						<view style="color: #999999;">{{item.attr_value}}</view>
					</view>
				</view>
				<view class="btns">
					<button type="warn" @click="closePopup()">{{$nw("完成")}}</button>
				</view>
			</view>

		</view>
	</view>

</template>

<script>
	export default {
		data() {
			return {
				/*是否可见*/
				Visible: false,
				list: []
			}
		},
		props: ['product_id', 'isShowAttr'],
		created() {
			this.Visible = this.isShowAttr
			this.getData()
		},
		methods: {
			/*关闭弹窗*/
			closePopup() {
				this.$emit('changeAttr', {})
			},

			/*获取数据*/
			getData() {
				let self = this;
				let product_id = self.product_id;
				uni.showLoading({
					title: self.$nw("加载中")
				});
				self._get(
					'product.product/findGoodsAttr', {
						product_id: product_id,
					},
					function(res) {
						self.list = res.data.info;
						uni.hideLoading();

					}
				);
			},
		}
	}
</script>

<style>
	.bottom-panel .popup-bg {
		position: fixed;
		top: 0;
		right: 0;
		bottom: 0;
		left: 0;
		background: rgba(0, 0, 0, .6);
		z-index: 98;
	}

	.bottom-panel .popup-bg .wechat-box {
		padding-top: var(--window-top);
	}

	.bottom-panel .popup-bg .wechat-box image {
		width: 100%;
	}

	.bottom-panel .content {
		position: fixed;
		width: 100%;
		bottom: 0;
		min-height: 200rpx;
		/* max-height: 900rpx; */
		background-color: #fff;
		transform: translate3d(0, 980rpx, 0);
		transition: transform .2s cubic-bezier(0, 0, .25, 1);
		bottom: env(safe-area-inset-bottom);
		z-index: 99;
		/* border-top-left-radius: 40rpx; */
		/* border-top-right-radius: 40rpx; */
	}

	.bottom-panel.open .content {
		transform: translate3d(0, 0, 0);
	}

	.bottom-panel.close .popup-bg {
		display: none;
	}

	.module-share .hd {
		height: 90rpx;
		text-align: center;
		line-height: 90rpx;
		font-size: 36rpx;
	}

	.module-share .item button,
	.module-share .item button::after {
		background: none;
		border: none;
	}


	.module-share .icon-box {
		width: 100rpx;
		height: 100rpx;
		border-radius: 50%;
		background: #f6bd1d;
	}

	.module-share .icon-box .iconfont {
		font-size: 60rpx;
		color: #FFFFFF;
	}

	.module-share .btns {
		margin-top: 30rpx;
	}

	.module-share .btns button {
		height: 90rpx;
		line-height: 90rpx;
		border-radius: 0;
		border-top: 1px solid #EEEEEE;
	}

	.module-share .btns button::after {
		border-radius: 0;
	}

	.module-share .share-friend {
		background: #04BE01;
	}

	.product-wraper {
		display: flex;
		flex-direction: column;
	}

	.product-item {
		/* height: 84rpx; */
		display: flex;
		align-items: center;
		margin: 0 20rpx;
		border-bottom: 1px solid rgb(245, 245, 245);
	}

	.product-item>view {
		padding: 20rpx 20rpx;
		/* height: 84rpx;
		line-height: 84rpx; */
	}

	.product-item>view:first-child {
		/* font-weight: bold; */
		color: black;
		width: 180rpx;
		white-space: pre-wrap;
	}

	.product-item>view:last-child {
		width: 570rpx;
	}
</style>