	...
	<view>
		...
		<view :class="'input-flex-column ' + (isIphoneX?'isIphoneX':'')" :style="'padding-bottom:' + (isIphoneX?88:20) + 'rpx'">
			<view class="input-text-voice-super">
				<input @confirm="chatInputSendTextMessage('text')"
				 @focus="chatInputBindFocusEvent" 
				 @input="chatInputGetValueEvent" 
				 class="chat-input-style" confirmType="send" 
				 maxlength="500" :style="'margin-left:' + (false ? 0 : 16) + 'rpx'" 
				 v-model="textMessage"
				...
				<block v-if="inputObj.inputType==='text'">
					...
					@click="chatInputSendTextMessage('text')" 
					class="chat-input-send-button-style" formType="submit" hoverClass="none" style="background-color: green ;color:#fff;padding:0rpx" >
					{{$nw("发送")}}
					</button>
				</block>
			...
			getChatId(){
				...
				function(res){
					if(res.code==1){
						...
						uni.setStorageSync("chatInter",d)
						self.i_time = d;
					}
				})
			},
