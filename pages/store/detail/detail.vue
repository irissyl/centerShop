<template>
	<view class="store-container bg-white" v-if="!loading">
		<view class="logo ww100 p-30-0 d-c-c" >
			<image v-if="storeDetail.logo && storeDetail.logo.file_path" :src="storeDetail.logo.file_path" mode="aspectFill"></image>
		</view>
		<view class="d-c-c d-c ww100" >
			<text class="f40">{{storeDetail.store_name}}</text>
			<text class="f28 gray9">{{$nw("营业时间")}}:{{storeDetail.shop_hours}}</text>
		</view>
