			...
			<view class="home-push" :class="type == 1 || type == 3 ? 'home-push-bg' : ''">
				...
				<template v-if="type == 1">
					<view class="image-text">
						<view class="pic"><image :src="form.file_path" mode="aspectFill"></image></view>
						<view class="title">{{ form.title }}</view>
						<view class="des">{{ form.remark }}</view>
						<view class="btns">
							<button type="primary" @click="gotoPage(form.link.url)">{{ form.des }}</button>
						</view>
					</view>
				</template>
				<!--纯图-->
				<template v-if="type == 2">
					<view class="image-only" @click="gotoPage(form.link.url)">
						<view class="pic"><image :src="form.file_path" mode="aspectFill"></image></view>
					</view>
				</template>
				<!--优惠券-->
				<template v-if="type == 3">
					<view class="cuopon">
						<view class="cuopon-title d-c-c" v-if="form.file_path!=null&&form.file_path!=''">
						...
						<view class="btns"><button type="primary" @click="getCoupon()">{{$nw("立即领取")}}</button></view>
					</view>
				</template>
			</view>
