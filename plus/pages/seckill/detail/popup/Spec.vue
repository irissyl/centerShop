		...
		<view class="main" v-on:click.stop>
			<view class="header">
				...
				<view class="price d-s-c">
					...
					<template v-else>
						...
						<text class="fb">-</text>
						<text class="f40 fb">{{ form.detail.seckill_high_price }}</text>
					</template>
				</view>
				<view class="stock">{{$nw("库存")}}：{{ form.show_sku.seckill_stock }}</view>
				<view class="p-20-0">{{ selectSpec }}</view>
			...
			<view class="body">
				...
				<view class="level-box count_choose">
					...
					<view class="d-s-c">
						<view class="icon-box minus d-c-c" @click="sub()" :class="{ 'num-wrap': !issub }"><text class="icon iconfont icon-jian"></text></view>
						<view class="text-wrap"><input type="text" v-model="form.show_sku.sum" value="" /></view>
						<view class="icon-box plus d-c-c" :class="{ 'num-wrap': !isadd }" @click="add()"><text class="icon iconfont icon-jia"></text></view>
					</view>
				</view> 
			...
			<view class="btns"><button type="primary" class="confirm-btn" @click="confirmFunc(form)">确认</button></view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		createdOrder() {
			...
			uni.navigateTo({
				url:
					'/main/pages/order/confirm-order/confirm-order?product_num=' +
					num +
					'&seckill_product_id=' +
					seckill_product_id +
					'&product_sku_id=' +
