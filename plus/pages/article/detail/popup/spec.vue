		...
		<view class="main" v-on:click.stop>
			<view class="header">
				...
				<view class="price">
					...
					<text class="old-price" v-if="form.show_sku.line_price!=null && form.show_sku.line_price > 0">{{$nw("¥")}}{{form.show_sku.line_price}}</text>
				</view>
				...
				<view v-if="form.specData !=null">
					<view class="specs mt20" v-for="(item_attr,attr_index) in form.specData.spec_attr" :key="attr_index">
						...
						<view class="specs-list">
							<button type="primary" :class="item.checked ? 'btn-red' : 'btn-gray-border'" v-for="(item,item_index) in item_attr.spec_items"
			...
			<view class="btns">
				<button type="primary" class="confirm-btn" @click="confirmFunc(form)">{{$nw("确认")}}</button>
			</view>
		</view>
	</view>
</template>
      ...
      this.setGlobalColor()
		...
		methods: {
			...
			createdOrder() {
				...
				uni.navigateTo({
					url: '/main/pages/order/confirm-order/confirm-order?product_id=' + product_id + '&product_num=' + product_num +
