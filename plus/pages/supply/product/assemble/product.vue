  ...
  <view>
    <view class="top-box">
      ...
      <view class="index-search-box nav-item">
        ...
        <button class="btn-blue" @click="addProduct(assemble_activity_id)">{{$nw("添加拼团商品")}}</button>
      </view>
            ...
            <view class="item">
              ...
              <view class="product-info">
                ...
                <view class="already-sale">
                  <text>{{$nw("已售")}}{{ item.join_num }}{{$nw("件")}}</text>
                  <text>{{$nw("团购库存")}}{{ item.stock }}{{$nw("件")}}</text>
                </view>
                <view class="already-sale">
                  <text>{{$nw("开团人数")}}{{ item.assemble_num }}{{$nw("件")}}</text>
                  <text>{{$nw("限购")}}{{ item.limit_num }}{{$nw("件")}}</text>
                </view>
                <view class="price">
                  {{$nw("拼团价")}}{{$nw("¥")}}
                  <text class="num">{{ item.assemble_price }}</text>
                </view>
              </view>
            </view>
            <view class="order-bts">
              <button @click="editProduct(item.assemble_product_id)">{{$nw("编辑")}}</button>
              <button @click="deleteProduct(index,item.assemble_product_id)">{{$nw("删除")}}</button>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      self.loading = true;
      self._get('plus.supply.assemble/product', {
        page: page || 1,
        ...
        uni.setNavigationBarTitle({
          title: res.data.detail.title + '-'+self.$nw("商品清单")
        })
      });
    },
    /*上下架*/
    ...
    deleteProduct(index, assemble_product_id) {
      let self = this;
      ...
      uni.showModal({
        ...
        success: function (res) {
          ...
          if (res.confirm) {
            ...
            self._post('plus.supply.assemble/del', {
    ...
    gotoSearch() {
      console.log(1);
      uni.navigateTo({
        url: '/main/pages/product/search/search?type=supply'
      });
    },
    addProduct(assemble_activity_id) {
      uni.navigateTo({
        // url: '/pages/supply/product/add'
        url: '/plus/pages/supply/product/list/list?assemble_activity_id=' + assemble_activity_id
      });
    },
    editProduct(assemble_product_id) {
      uni.navigateTo({
        url: '/plus/pages/supply/product/assemble/product/edit?assemble_product_id=' + assemble_product_id
      });
    },
    /*可滚动视图区域到底触发*/
    scrolltolowerFunc() {
