    ...
    <form @submit="formSubmit" @reset="formReset">
      <view class="bg-white p-0-30 f30">
        <view class="d-s-c border-b-e">
          ...
          <input class="ml20 flex-1 p-30-0" name="product_name" type="text" v-model="form.product_name" :placeholder="$nw('请输入')"/>
        </view>
        <view class="d-s-c border-b-e">
          <text class="key-name">{{$nw("商品编号")}}：</text>
          <input class="ml20 flex-1 p-30-0" name="product_no" type="text" v-model="form.product_no" :placeholder="$nw('请输入')"/>
        </view>
        <view class="d-s-c border-b-e">
        ...
        <view class="d-s-c border-b-e">
          ...
          <input class="ml20 flex-1 p-30-0" name="product_price" type="text" v-model="form.sku.product_price" :placeholder="$nw('请输入')"/>
        </view>
        <view class="d-s-c border-b-e">
          <text class="key-name">{{$nw("供货价")}}：</text>
          <input class="ml20 flex-1 p-30-0" name="product_supply_price" type="text" v-model="form.sku.product_supply_price" :placeholder="$nw('请输入')"/>
        </view>
        <view class="d-s-c border-b-e">
        ...
        <view class="d-s-c border-b-e" @click="gotoContent">
          ...
          <view class="m20 flex-1 p-30-0" style="width: 80%;text-align: right;">{{$nw("详情")}}</view>
        </view>
      </view>
      <view class="p30">
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      self._get('plus.supply.product/edit', {product_id: product_id}, function (res) {
            ...
            var delivery_id = self.form.delivery_id
            delivery.forEach(function (item) {
              if (item.delivery_id == delivery_id) {
                self.delivery_name = item.name
              }
            })
            if (res.data.model.category_id) {
              self.cat_name = res.data.model.category.name
            }
            uni.setStorageSync('goods_content', self.form.content)
            uni.hideLoading();
    ...
    formSubmit: function (e) {
      ...
      formdata = self.formatModel(self.model, formdata);
      // formdata.image = self.ImgKeepId(formdata.image);
      var params = {product_id: self.product_id};
      ...
      self._post('plus.supply.product/edit', params, function (res) {
        uni.hideLoading();
        uni.showModal({
          title: self.$nw("提示"),
          content: res.msg,
