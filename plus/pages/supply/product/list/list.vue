  ...
  <view>
    <view class="top-box">
      ...
      <view class="index-search-box nav-item">
        ...
        <button class="btn-blue" @click="addProduct()">{{ $nw("添加")}}</button>
      </view>
        ...
        <view class="list">
          <view v-for="(item, index) in listData" :key="index">
            ...
            <view class="order-bts">
              <button v-if="assemble_activity_id>0" class="btn-red" :style="[active]" @click="addAssemble(item.product_id)">{{ $nw("加入拼团")}}</button>
              <button v-if="item.product_status.value==10" class="btn-blue" @click="updateProduct(index,item.product_id)">{{ $nw("刷新")}}</button>
              <button v-if="item.product_status.value==10" class="btn-blue" @click="shelvesProduct(index,20,item.product_id)">{{ $nw("上架")}}</button>
              <button v-if="item.product_status.value==20" class="btn-red"  @click="shelvesProduct(index,10,item.product_id)">{{ $nw("下架")}}</button>
              <button @click="editProduct(item.product_id)">{{ $nw("编辑")}}</button>
              <button @click="deleteProduct(index,item.product_id)">{{ $nw("删除")}}</button>
...
export default {
  ...
  methods: {
    ...
    /*上下架*/
    shelvesProduct(index, product_status, product_id) {
      let self = this;
      let msg = product_status == 20 ?self.$nw("确认下架？") : self.$nw("确认上架？");
      uni.showModal({
        title: self.$nw("提示") ,
        content: msg,
        success: function (res) {
          ...
          if (res.confirm) {
            ...
            self._post('plus.supply.product/shelvesProduct', {
              product_id: product_id,
              product_status: product_status,
            }, function (res) {
              self.listData[index].product_status.value = product_status
              uni.hideLoading();
            });
          } else if (res.cancel) {
    ...
    updateProduct(index, product_id) {
      ...
      uni.showModal({
        title: self.$nw("提示"),
        content: self.$nw("确认刷新商品？") ,
        success: function (res) {
          if (res.confirm) {
            uni.showLoading({
              title:  self.$nw("处理中") 
            });
            self._post('plus.supply.product/updateProduct', {
              product_id: product_id,
            }, function (res) {
              self.restoreData();
              self.getData();
      ...
      uni.showModal({
        ...
        success: function (res) {
          ...
          if (res.confirm) {
            ...
            self._post('plus.supply.product/delete', {
              product_id: product_id,
            }, function (res) {
              console.log(res)
    ...
    /*跳转产品列表*/
    addAssemble(e) {
      let url = '/plus/pages/supply/product/assemble/product/add?product_id=' + e + '&assemble_activity_id=' + this.assemble_activity_id
      this.gotoPage(url);
    },
    ...
    gotoSearch() {
      console.log(1);
      uni.navigateTo({
        url: '/main/pages/product/search/search?type=supply&assemble_activity_id=' + this.assemble_activity_id
      });
    },
    addProduct() {
      uni.navigateTo({
        url: '/plus/pages/supply/product/add'
      });
    },
    editProduct(product_id) {
      uni.navigateTo({
        url: '/plus/pages/supply/product/edit?product_id=' + product_id
      });
    },
    /*可滚动视图区域到底触发*/
    scrolltolowerFunc() {
