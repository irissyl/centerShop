	...
	<view class="fight-group-detail" v-if="!loadding">
		...
		<view class="product-info p30 d-s-c bg-white">
			...
			<view class="info flex-1 ml30 f30">
				...
				<view class="mt20">
					<text v-if="billData.activity.assemble_type==0">{{productData.assemble_num}}{{$nw("人团")}}：</text>
					<text v-else>{{$nw("库存团")}}：</text>
					<text class="ml30 red">{{$nw("¥")}}</text>
		...
		<view class="fight-users mt20 bg-white">
			...
			<view class="d-c-c mt30 gray3">
				<view v-if="billData.activity.assemble_type==0">
					{{$nw("还差")}}<text class="fb">{{dif_people}}</text>{{$nw("人成团")}}
				</view>
				<view v-else>
					{{$nw("还差")}}<text class="fb">{{dif_people}}</text>{{$nw("件成团")}}
				</view>
		...
		<view class="leader-info d-b-c p30">
			...
			<view class="gray9">
				...
				<text class="ml10">{{$nw("开团")}}</text>
			</view>
		</view>

		<!--底部按钮-->
		<view class="fight-group-btns">
			<button v-if="!reallyAssemble" type="default" class="btn-red" :style="[active]" @click="gotoAssemble(billData)">{{$nw("去拼团")}}</button>
			<button v-else type="default" class="btn-red" :style="[active]" open-type="share" @click="shareFunc">{{$nw("分享拼团")}}</button>
		</view>
      ...
      active() {
		...
		methods: {
			...
			getData() {
				...
				self._get(
					...
					function(res) {
						...
						if(res.data.product){
							self.product_id=res.data.product.product_id;
						}
						
						if(self.billData){
							if(self.billData.activity.assemble_type==0 && self.billData.billUser){
								...
								self.dif_people = res.data.product.stock
							}
						}
						...
						//#endif
						if(self.billData){
							if(self.billData.creator_id ==  self.getUserId()){
								//作为开团者 不需要参与也可以看到分享按钮 
								self.reallyAssemble = true
							}else{
			...
			onShareAppMessage() {
				...
				return {
					title: self.$nw("【仅限")+self.dif_people+self.$nw("个名额】，快来参与拼团吧"),
					path: '/plus/pages/assemble/fight-group-detail/fight-group-detail?' + params
				};
			},

			/*去拼团*/
			gotoAssemble(e){
				let url='plus/pages/assemble/detail/detail?assemble_product_id=' + e.assemble_product_id+'&assemble_bill_id='+e.assemble_bill_id
				this.gotoPage(url);
			}
		}
	}
