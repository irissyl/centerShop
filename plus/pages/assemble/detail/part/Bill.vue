<template>
	<view class="bg-white mt20" v-if="bill.length > 0">
		<view class="group-hd p-0-20 d-a-c border-b">
			<view class="left"><text class="f30">{{$nw("这些人刚刚购买成功，可参与拼单")}}</text></view>
		...
		<view class="group-bd bill-user-list">
			<view class="item d-b-c p20" v-for="(item, index) in bill" :key="index">
				<view class="userinfo d-s-c">
					<view class="photo"><image :src="item.user.avatarUrl" mode="widthFix"></image></view>
					<text class="ml10">{{ item.user.nickName }}</text>
				</view>
				<view class="btns d-s-c">
					<view class="d-s-c d-c">
						<text class="ml10">{{$nw("还差")}}{{ item.dif_people }}{{$nw("人成团")}}</text>
						<view class="gray9">
							<Countdown :config="rturnObjec(item)"></Countdown>
						</view>
					</view>
					<button type="primary" class="btn-red ml20" @click="goBill(item)">{{$nw("去拼单")}}</button>
				</view>
			</view>
		</view>
	</view>
