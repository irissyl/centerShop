<template>
	<view :class="Visible ? 'product-popup open' : 'product-popup close'" @touchmove.stop.prevent="" @click="closePopup">
		...
		<view class="main" v-on:click.stop>
			<view class="header">
				...
				<view class="price d-s-c">
					<template v-if="form.specData == null||isAll">
						...
						<text class="old-price" v-if="form.show_sku.line_price && form.show_sku.line_price > 0">{{$nw("¥")}}{{ form.show_sku.line_price }}</text>
					</template>
					<template v-else>
				...
				<view class="stock">{{$nw("库存")}}：{{ form.show_sku.assemble_stock }}</view>
			...
			<view class="btns"><button type="primary" class="confirm-btn" @click="confirmFunc(form)">{{$nw("确认")}}</button></view>
		</view>
	</view>
</template>
		...
		createdOrder() {
			...
			uni.navigateTo({
				url:
					...
					assemble_product_sku_id +
					'&assemble_bill_id=' +
					this.form.show_sku.assemble_bill_id +
					'&order_type=assemble'
			});
		},
