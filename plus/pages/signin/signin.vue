<template>
	<view class="signin-container" v-if="!loadding" 
	:style="'background:'+getMainColor()+';'">
		<view class="sigin-title d-c-c"><text>{{$nw("签到有礼")}}</text></view>
		<view class="sign-main">
			<view class="sign-main-bg" :style="'background:'+getSecondBackgroundColor()+';'">
				<view class="sign-head f28" >
					<view class="small" v-if="reward.length > 0 && reward[0] >= 1">
						({{$nw("再签到")}}
						<text class="p-0-10 yellow fb">{{ reward[0] }}</text>
						{{$nw("天可得到")}}
						<text class="p-0-10 yellow fb">{{ integral[reward[0]] }}</text>
						{{$nw("积分")}})
					</view>
					<view class="small" v-if="reward.length > 0 && reward[0] == 1 && !is_sign">({{$nw("今天签到即可得到额外奖励")}})</view>
				</view>
				<view class="sign-calendar-wrap">
					<view class="date-text">{{ today_year }}{{$nw("年")}}{{ today_month }}{{$nw("月")}}</view>
			...
			<view class="pt30 d-c-c f34">
				{{$nw("你已经累计签到")}}
				<text class="p-0-10 yellow fb">{{ days }}</text>
				{{$nw("天")}}
			</view>
			<view class="btns">
				<button type="primary" @click="onSign" v-if="!is_sign"
				style="background: linear-gradient(to right,#f2b237,#ffda98);"
				>{{$nw("立即签到")}}</button>
				<view v-else class="d-c-c yellow f40 fb">
					<text class="iconfont icon-queren mr10"></text>{{$nw("今日已签到")}}</view>
			</view>
		</view>

		<!--活动规则-->
		<view class="rule-btn" @click="gotoRuleFunc"
		style="background: linear-gradient(to right,#f2b237,#ffda98);"
		>{{$nw("活动规则")}}</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		gotoRuleFunc() {
			uni.navigateTo({
				url: '/plus/pages/signin/rule/rule'
			});
		}
	}
};
