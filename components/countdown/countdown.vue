<template>
	...
	<view class="countdown">
		<template v-if="config.type == null">
			...
			<text v-if="status == 1">{{$nw("活动开始时间")}}：</text>
			<text v-if="status == 2">{{$nw("活动结束时间")}}：</text>
			<text class="box">{{ day }}</text> 
			<text class="p-0-10">{{$nw("天")}}</text>
			<text class="box">{{ hour }}</text>
			<text class="p-0-10">{{$nw("时")}}</text>
			<text class="box">{{ minute }}</text>
			<text class="p-0-10">{{$nw("分")}}</text>
			<text class="box">{{ second }}</text>
			<text class="p-0-10">{{$nw("秒")}}</text>
		</template>
