	...
	<view class="content">
		...
		<view class="region-box" style="padding-bottom: 200rpx;" v-if="!is_loading" :style="'height:'+ heightCot + '%'">
			...
			<view class="map-txt">
				...
				<text v-if="area != $nw('请选择') && streetsData.length>0">-</text>
				<text class="w-25" v-if="area != $nw('请选择') && streetsData.length>0" :class="showIndex== 3 ?'select':''" @click="anewSelect(3)">{{street}}</text>
			</view>
			<view class="main-box">
