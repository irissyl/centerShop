<template>
	<view class="diy-article">
		<!--文章列表-->
		<Label :item="item3"   :name="itemData.params.name" v-if="itemData.data.length > 0"></Label>
...
export default {
	...
	mounted() {
		var self = this;
		setTimeout(function() {
			self.item3.color = self.getMainTextColor()
	...
	methods: {
		...
		/*跳转页面*/
		gotoPageFunc(e) {
			let _url='plus/pages/article/detail/detail?article_id=' + e.article_id;
			gotopage(_url);
		}
	}
};
