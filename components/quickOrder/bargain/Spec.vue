	...
	<view>
		<view  style="padding: 0rpx;" :class="Visible?'product-popup open':'product-popup close'">
			<view class="header">
				...
				<view class="stock">{{$nw("库存")}}：{{ form.show_sku.bargain_stock }}</view>
	...
	methods: {
		...
		updateSpecProduct() {
			...
			if (typeof skuItem === 'object') {
				...
				self.form.show_sku.product_sku_id = skuItem.product_sku_id;
		...
		createdOrder() {
			...
			self._get(
				...
				function(res) {
					...
					self.$emit('close', {});
					uni.navigateTo({
						url: '/plus/pages/bargain/haggle/haggle?bargain_task_id=' + bargain_task_id + '&order_type=bargain'
					});
				}
			);
		},
