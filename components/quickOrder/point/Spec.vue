<template>
	<view style="padding: 0rpx;" :class="Visible?'product-popup open':'product-popup close'">
		<view class="header">
				...
				<view class="price d-s-c" style="margin-top: 30rpx;">
					<text class="f24">{{$nw("所需积分")}}</text>
					<text class="num fb f34">{{ form.show_sku.point_num }}</text>
					<text>+</text>
					<text class="old-price">{{$nw("¥")}}{{ form.show_sku.point_money }}</text>
				</view>
				<view class="stock" style="margin-top: 10rpx;">{{$nw("库存")}}：{{ form.show_sku.point_stock }}</view>
