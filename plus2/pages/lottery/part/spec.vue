<template>
	<view :class="Visible ? 'product-popup open' : 'product-popup close'" @touchmove.stop.prevent="" @click="closePopup">
		...
		<view class="main" v-on:click.stop>
					...
					<scroll-view scroll-y="true" class="specs mt20" style="max-height: 600rpx;" v-if="form.specData != null">
						<view class="specs mt20" v-for="(item_attr, attr_index) in form.specData.spec_attr" :key="attr_index">
							<view class="specs-hd p-20-0">
								...
								<text class="ml10 red" v-if="form.productSpecArr[attr_index] == null">{{$nw("请选择")}} {{ item_attr.group_name }}</text>
							</view>
			...
			<view class="btns"><button type="primary" class="confirm-btn" @click="closePopup()">{{$nw("选好了")}}</button></view>
		</view>
	</view>
</template>
		...
		isPopup: function(n, o) {
			if (n != o) {
				...
				this.form.specData.spec_attr.forEach((item, index) => {
					item.spec_items.forEach((citem, cindex) => {
						if (!citem.checked) {
							this.form.productSpecArr[index] = null;
							// console.log(citem);
							// citem.checked = false;
							this.$set(this.form.specData.spec_attr[index].spec_items[cindex], 'checked', false);
						} else {
							// citem.checked = true;
							this.$set(this.form.specData.spec_attr[index].spec_items[cindex], 'checked', false);
							this.form.productSpecArr[index] = citem.item_id;
						}
					});
				});
			}
