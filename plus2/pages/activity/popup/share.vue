<template>
	...
	<view class="bottom-panel" :class="Visible?'bottom-panel open':'bottom-panel close'">
		<view class="popup-bg" @click="closePopup">
					...
					<view class="d-c-c">
						...
						<!-- <view class="item flex-1 d-c-c d-c">
							<button @click="genePoster">
								...
								<text class="pt20">{{$nw("生成海报")}}</text>
							</button>
						</view> -->
					</view>
				</view>
				<view class="btns">
					<button type="default" @click="closePopup(1)">{{$nw("取消")}}</button>
				</view>
			</view>
		...
		methods: {
			...
			share: function() {
				//#ifdef H5
					...
					this.thumbsup_handle(this, this.article_id, "share", "article");
				//#endif
			},
			//生成海报
			genePoster: function() {
				...
				self._get('product.product/poster', {
					...
					self.closePopup(2);
					self.thumbsup_handle(self, self.article_id, "share", "article");
				}, null, () => {
					uni.hideLoading();
				});
			},
