	...
	<scroll-view scroll-y="true" :style="'height:' + scrollviewHigh + 'px;'+'background:'+getMainColor()+';'"
		...
		<view class="search-box" :style="'background:'+getMainColor()+';'">
			<view class="search-item" :style="'background:'+getSecondBackgroundColor()+';'">
				...
				<input @confirm="search" v-model="keyword" class="ml-md flex-1" confirmType="search"
					placeholderClass="placeholder" placeholder="请输入校友企业名称/手机号/联系人" type="text"></input>
			</view>
		</view>
			...
			<view class="yuyue">
				...
				<view class="yuyue_2">
					...
					<view class="yuyue_2_3" v-if="item.business">
						<text style="font-size: 30rpx;">主营：</text> <text style="color: #888783;font-size: 24rpx;">
							{{item.business}}</text>
					</view>
		...
		<template v-if="list.length <=0">
			<view class="none-data-box">
				<text :style="'color:'+getTextColorWhite()+';'">未获取到数据</text>
		...
		methods: {
			...
			getTab(type) {
				if (type == 1) {
					...
					return this.categoryaArray && this.categoryaArray[this.categorya_index]? this.categoryaArray[this.categorya_index].categorya_id : -1;
				} else if (type == 3) {
