...
<script>
	export default {
		...
		onLoad(option) {
			...
			this.supply_id = option.supply_id
			let _this = this
		...
		methods: {
			...
			getData() {
				...
				_this._get('plus.supply.supply/getCategory', {}, function(res) {
			...
			gotoList(e) {
				...
				if (this.source != 'tz' && this.source != 'supply') {
					prevPage.$vm.currentIndustry = e.name;
					prevPage.$vm.staffInfo.category_id = e.category_id;
				} else if (this.source == 'tz') {
					prevPage.$vm.currentIndustry_tz = e.name;
					prevPage.$vm.staffInfo.setting.category_id = e.category_id;
				} else if (this.source == 'supply') {
					prevPage.$vm.currentIndustry = e.name;
					prevPage.$vm.category_id = e.category_id;
					//修改商家分类
					console.log("self.supply_id", this.supply_id)
					if (this.supply_id > 0) {
						this._post('plus.supply.supply/updCategory', {
